...

{
  ...
  outputs = inputs@{ self, nixpkgs, nix-darwin, home-manager, sops-nix, nix-private }:
  let
    ...
    configuration = {pkgs, ... }: {
        ...
        environment.systemPackages = with pkgs; [ 
          git openssh zsh bash coreutils findutils gawk gnugrep gnused rsync curl 
          openssl cmake gnumake pkg-config python313 nodejs go rustup starship neovim tmux
        ];
      ...
      homeconfig = { pkgs, config, ... }: {
        ...
        home.packages = with pkgs; [ sops age ssh-to-age oh-my-zsh openssh starship ];
