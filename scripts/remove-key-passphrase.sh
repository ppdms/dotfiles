...
if [ $# -ne 1 ]; then
    ...
    echo "Example: $0 ssh_main"
    exit 1
fi

SECRET_NAME="$1"
SECRETS_FILE="$HOME/.local/state/nix-config/secrets.sops.yaml"
...
if [ $? -eq 0 ]; then
    ...
    echo "  sops ~/.local/state/nix-config/secrets.sops.yaml"
    echo "  ~/.config/nix/scripts/secrets-edit-backup.sh --seal-only"
    echo ""
